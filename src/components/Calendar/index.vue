...
export default {
    ...
    computed: {
        ...
        weeks() {
            ...
            let preMonthTotalDays = getTotalDaysArr(year, preMonth).map(day => {
                let dateTime = new Date(year, preMonth - 1, day);
                let item = {
                    year,
                    month: preMonth,
                    day,
            ...
            firstWeek.unshift(...preMonthTotalDays.slice(firstWeek.length - 7));
            weeks[0] = firstWeek;
            // 下一月日期
            let lastWeek = weeks[weeks.length - 1];
            let nextMonth = month + 1;
            let nextMonthTotalDays = getTotalDaysArr(year, nextMonth).map(
                day => {
                    let dateTime = new Date(year, nextMonth - 1, day);
                    let item = {
                        year,
                        month: nextMonth,
                        day,
    ...
    methods: {
        ...
        getDayClass(day) {
            let tmp = {};
            if (day.timestamp == this.$data.activeItem.timestamp) {
                tmp.active = true;
            }
            if (day.data) {
                tmp["has-data"] = true;
