        ...
        <div class="overflow-container">
            ...
            <template v-if="discuz.isLogin">
                ...
                <ul class="area" v-for="(area,i) of areaList" :key="i" :data-title="area.name">
                    <router-link v-for="(form,ii) of area.value" :key="ii" :to="{name: 'DiscuzForumView', params: { url: targetHost + form.value }}" tag="li">
                      {{form.name}}
                    </router-link>
                </ul>
            </template>
            <a class="btn switch" @click="switchHost">切换源(当前:{{discuz.HOST.includes('3322')?'3322':'herokuapp'}})</a>
        </div>
    </div>
</template>
<script>
  ...
  methods: {
      ...
      data.recentTopics &&
        data.recentTopics.forEach(item => {
          if (
            item &&
            item.title ==
              `${data.username}/${new Date().getMonth() + 1}月份/打卡签到帖`
          ) {
            ...
            item &&
            item.title ==
              `${data.username}/${new Date().getMonth()}月份/打卡签到帖`
          ) {
            signInfo.prevMonthSignThreadLastPostUrl = item.lastPostUrl;
          }
        });
    ...
    async uploadEveryMonthSignInfo() {
      if (
        ...
        let postData1 = {
          httpConfig: {
            url: `${targetHost + signInfo.prevMonthSignThreadLastPostUrl}`,
            method: "post",
            responseType: "arraybuffer"
          },
          encoding: "gbk",
    ...
    async getIndexPageJson() {
      ...
      if (sessionStorage.getItem(url)) {
        ...
        let {
          data: { data }
        } = await http.post(`${this.discuz.HOST}/api/html2Json`, postData);
        this.$store.commit("SET_LOADING_STATUS", false);
        pageData = data;
